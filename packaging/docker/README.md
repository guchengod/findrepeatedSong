...
1. 下载与设备 CPU 匹配的 Docker 镜像归档，并导入：
   ...
   ```bash
   docker load -i findrepeatedsong-linux-amd64-0.1.1.tar
   ```

   ARM 设备（包括 Apple Silicon Docker Desktop）使用 `findrepeatedsong-linux-arm64-0.1.1.tar`。
...
4. 打开 `http://localhost:8080`。容器内可见的音乐目录为 `/music`。
